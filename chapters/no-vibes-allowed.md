...
이 관계를 식으로 보면, 잘못된 정보는 제곱이라 조금만 들어가도 성능이 크게 떨어지고, Size는 분모라서 컨텍스트가 길어질수록 성능이 나빠집니다.

그래서 팀은 프롬프트 문장력보다 컨텍스트 품질 관리 체계를 먼저 갖추는 것이 우선입니다.
...
연구 단계에서는 코드를 많이 읽는 것이 아니라, 코드 기반 사실을 압축합니다.
계획 단계에서는 의도·순서·검증을 압축하고, 여기서 인간이 검토해 AI와 의도를 정렬합니다.
구현 단계는 이미 승인된 계획이 있으므로, 에이전트는 저컨텍스트로 실행만 하면 됩니다.

RPI를 제대로 운영하려면 단계 이름보다 각 단계에서 나오는 산출물의 품질이 중요합니다.
다음 슬라이드에서 연구·계획·구현 각 단계의 산출물 기준을 보겠습니다.
-->
...
리뷰 순서를 바꿔야 합니다. 계획과 근거를 먼저 검토한 뒤 코드 디프를 보면, 인지 부하를 줄이고 설계 판단에 시간을 쓸 수 있습니다.
-->
